...
![平方误差](resources/8.Regression/LinearR_18.png)

用矩阵表示还可以写做 ![平方误差_2](resources/8.Regression/LinearR_19.png) 。如果对 w 求导，得到 ![平方误差_3](resources/8.Regression/LinearR_20.png) ，令其等于零，解出 w 如下（具体求导过程为: http://blog.csdn.net/nomadlx53/article/details/50849941 ）:

![回归系数的最佳估计计算公式](resources/8.Regression/LinearR_1.png)
...
![线性回归数据示例图](resources/8.Regression/LinearR_2.png "线性回归数据示例图")
...
![线性回归数据效果图](resources/8.Regression/LinearR_3.png "线性回归数据效果图")
...
![局部加权线性回归回归系数公式](resources/8.Regression/LinearR_21.png)

与 kNN 一样，这种算法每次预测均需要事先选取出对应的数据子集。该算法解出回归系数 w 的形式如下: 

![局部加权线性回归回归系数公式](resources/8.Regression/LinearR_4.png)

其中 w 是一个矩阵，用来给每个数据点赋予权重。

LWLR 使用 “核”（与支持向量机中的核类似）来对附近的点赋予更高的权重。核的类型可以自由选择，最常用的核就是高斯核，高斯核对应的权重如下: 

![局部加权线性回归高斯核](resources/8.Regression/LinearR_23.png)
...
![参数k与权重的关系](resources/8.Regression/LinearR_6.png)
...
![局部加权线性回归数据示例图](resources/8.Regression/LinearR_2.png)
...
![局部加权线性回归数据效果图](resources/8.Regression/LinearR_7.png)
...
如果数据的特征比样本点还多应该怎么办？是否还可以使用线性回归和之前的方法来做预测？答案是否定的，即我们不能再使用前面介绍的方法。这是因为在计算 ![矩阵求逆](resources/8.Regression/LinearR_8.png) 的时候会出错。
...
简单来说，岭回归就是在矩阵 ![矩阵_1](resources/8.Regression/LinearR_9.png) 上加一个 λI 从而使得矩阵非奇异，进而能对 ![矩阵_2](resources/8.Regression/LinearR_10.png ) 求逆。其中矩阵I是一个 n * n （等于列数） 的单位矩阵，
对角线上元素全为1，其他元素全为0。而λ是一个用户定义的数值，后面会做介绍。在这种情况下，回归系数的计算公式将变成：

![岭回归的回归系数计算](resources/8.Regression/LinearR_11.png )

岭回归最先用来处理特征数多于样本数的情况，现在也用于在估计中加入偏差，从而得到更好的估计。这里通过引入 λ 来限制了所有 w 之和，通过引入该惩罚项，能够减少不重要的参数，这个技术在统计学中也叫作 `缩减(shrinkage)`。

![岭回归](resources/8.Regression/LinearR_22.png)
...
![岭回归的运行效果](resources/8.Regression/LinearR_12.png)
...
![逐步线性回归运行效果](resources/8.Regression/LinearR_15.png)
...
![生成公式](resources/8.Regression/LinearR_16.png)
...
![偏差方差图](resources/8.Regression/LinearR_17.png)

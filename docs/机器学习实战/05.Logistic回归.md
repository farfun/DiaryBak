...
![Sigmoid 函数计算公式](resources/5.Logistic/LR_1.png)
...
![Sigmoid 函数在不同坐标下的图片](resources/5.Logistic/LR_3.png)
...
![Sigmoid 函数计算公式](resources/5.Logistic/LR_2.png)

如果采用向量的写法，上述公式可以写成 ![Sigmoid 函数计算公式向量形式](resources/5.Logistic/LR_4.png) ，它表示将这两个数值向量对应元素相乘然后全部加起来即得到 z 值。其中的向量 x 是分类器的输入数据，向量 w 也就是我们要找到的最佳参数（系数），从而使得分类器尽可能地精确。为了寻找该最佳参数，需要用到最优化理论的一些知识。我们这里使用的是——梯度上升法（Gradient Ascent）。
...
![梯度上升计算公式](resources/5.Logistic/LR_5.png)

这个梯度意味着要沿 x 的方向移动 ![f(x, y)对x求偏导](resources/5.Logistic/LR_6.png) ，沿 y 的方向移动 ![f(x, y)对y求偏导](resources/5.Logistic/LR_7.png) 。其中，函数f(x, y) 必须要在待计算的点上有定义并且可微。下图是一个具体的例子。

![梯度上升](resources/5.Logistic/LR_8.png)
...
![梯度上升迭代公式](resources/5.Logistic/LR_9.png)
...
![梯度下降迭代公式](resources/5.Logistic/LR_10.png)


**局部最优现象 （Local Optima）**

![梯度下降图_4](resources/5.Logistic/LR_20.png)
...
![方向导数与梯度](resources/5.Logistic/方向导数和梯度.png)
...
![简单数据集绘制在图上](resources/5.Logistic/LR_11.png)
...
![f(w)对w求一阶导数](resources/5.Logistic/LR_21.png)
...
![回归系数与迭代次数的关系图](resources/5.Logistic/LR_12.png)
...
![改进随机梯度下降结果图](resources/5.Logistic/LR_13.png)
